...
TEST(CtorMove, True) {
  ...
  ASSERT_TRUE(matrix_a.GetColumns() == 0);
  ASSERT_TRUE(matrix_b.GetRows() == 2);
  ASSERT_TRUE(matrix_b.GetColumns() == 2);
}
...
TEST(Inverse, True) {
  ...
  S21Matrix inversed = matrix_a.InverseMatrix();
  ASSERT_TRUE(inversed.EqMatrix(result));
...
TEST(Get, True) {
  ...
  ASSERT_EQ(matrix_a.GetColumns(), 3);
}
TEST(Set, True) {
  S21Matrix matrix_a(3, 3);
